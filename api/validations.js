...
const validateBodyForEmail = (form) => {
    let formErrors = {};
    formErrors.name = verifyName(form.name);
    formErrors.email = verifyEmail(form.email);
    formErrors.order = !form.order.length ? "cart cant be empty" : ""
    formErrors.total = form.total > 0 ? "": "wrong total"
    return formErrors;
}

const userExists = async (user, users) => {
    var returnValue = false;
    await users.findOne({ email: user.email})
        .then((usr) => {
            returnValue = usr !== null ? true : false;
        })
        .catch((err) => {
            console.log(err);
        })
